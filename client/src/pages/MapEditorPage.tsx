...
export function MapEditorPage() {
  ...
  function handleVertexClick(index: number) {
    // Клик по первой точке зоны замыкает её и предлагает сохранить.
    if (index === 0 && activeType.kind === "zone" && draft.length >= 3) {
      // Зона уже автоматически замыкается визуально (полигон), поэтому здесь
      // просто фокус на кнопке сохранения через toast.
      toast({
        title: "Зона замкнута",
        description: name.trim() ? "Нажмите «Сохранить»." : "Введите название и нажмите «Сохранить».",
      });
      return;
    }
    // Клик по последней добавленной — удаляем её (быстрая отмена).
    if (index === draft.length - 1) {
      setDraft((d) => d.slice(0, -1));
      return;
    }
    // Клик по любой другой вершине — удаляем её из массива.
