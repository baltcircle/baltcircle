...
// iOS вставляет между заголовком и телом свою строку «from <имя приложения>»,
// поэтому на карточку остаётся одна строка заголовка и одна-две строки тела.
// Заголовок держим коротким и с самым важным — числом оставшихся минут.
function texts(stage: ExpiryStage, bikeId: string): { title: string; body: string } {
  switch (stage) {
    case "warn10":
      return {
        title: "Аренда: осталось 10 мин.",
        body: `${bikeId} — продлите аренду или завершите поездку на парковке.`,
      };
    case "warn5":
      return {
        title: "Аренда: осталось 5 мин.",
        body: `${bikeId} — продлите аренду или завершите поездку на парковке.`,
      };
    case "overtime":
      return {
        title: "Аренда: время истекло",
        body: `${bikeId} — идёт поминутная тарификация. Завершите поездку или продлите аренду.`,
      };
  }
}
